...
#	define _DT_ARRAY_ELEM_PATH(cmd, arr, path, idx, val) \
	do { \
		struct dt_value_t* __tmp = (cmd)->target; \
		dt_array_elem_at(DT_DUMMY_DATATREE, (arr), (idx), &(val)); \
		(cmd)->target = (struct dt_value_t*)val; \
		_dt_access_by_path(DT_DUMMY_DATATREE, (struct dt_value_t*)(val), (path), (struct dt_value_t**)&(val)); \
		(cmd)->target = __tmp; \
	} while(0)
...
typedef struct _dt_command_t {
	...
	int parsed;
	struct dt_value_t* target;
	struct dt_value_t* got;
...
extern void _dt_value_assign(struct dt_value_t* v, const char* s);
...
static int _dt_value_is(struct dt_value_t* val, unsigned int type);
...
static _dt_expr_node_t* _dt_expr_operate(dt_datatree_t d, struct dt_value_t* target, _dt_expr_node_t* a, _dt_expr_node_t* theta, _dt_expr_node_t* b);
...
static dt_query_status_t _dt_eval_expr(dt_datatree_t d, struct dt_value_t* target, _dt_expr_node_t* e, int* ret);
...
static dt_query_status_t _dt_access_by_path(dt_datatree_t d, struct dt_value_t* target, _dt_path_node_t* path, struct dt_value_t** ret);
...
static dt_query_status_t _dt_make_new_object(dt_datatree_t d, dt_parse_error_handler_t eh, struct dt_value_t* target, _dt_ctor_t* ctor, struct dt_value_t** ret);
...
int _dt_value_is(struct dt_value_t* val, unsigned int type) {
	int result = 0;
...
xpl_status_t _dt_query_from(xpl_context_t* _s) {
	...
	if(cmd->parsed) {
		...
		if(stat->oprand.type == DT_STATEMENT_PATH) {
			...
			cmd->target = (struct dt_value_t*)middle;
		} else {
		...
		cmd->got = (struct dt_value_t*)result_object;
...
xpl_status_t _dt_query_select(xpl_context_t* _s) {
	...
	if(cmd->parsed) {
		...
		struct dt_value_t* target = cmd->target;
		stat = &cmd->statements[*cmd->statement_cursor];
		if(stat->oprand.type == DT_STATEMENT_PATH) {
...
_dt_expr_node_t* _dt_expr_operate(dt_datatree_t d, struct dt_value_t* target, _dt_expr_node_t* a, _dt_expr_node_t* theta, _dt_expr_node_t* b) {
	_dt_expr_node_t* result = NULL;
	union { struct dt_value_t* val; int b; } val_a, val_b;
...
dt_query_status_t _dt_eval_expr(dt_datatree_t d, struct dt_value_t* target, _dt_expr_node_t* e, int* ret) {
...
dt_query_status_t _dt_access_by_path(dt_datatree_t d, struct dt_value_t* target, _dt_path_node_t* path, struct dt_value_t** ret) {
	_dt_path_node_t* node = NULL;
	...
	while(node) {
		...
		case DT_PATH_NODE_OBJECT_KEY: {
				...
				if(node->type == DT_PATH_NODE_BUILDIN_THIS) {
					...
					dt_create_value(DT_DUMMY_DATATREE, &got.value, NULL, NULL, DT_LONG, len);
					*ret = (struct dt_value_t*)got.value;
...
dt_query_status_t _dt_make_new_object(dt_datatree_t d, dt_parse_error_handler_t eh, struct dt_value_t* target, _dt_ctor_t* ctor, struct dt_value_t** ret) {
	size_t i = 0;
...
const char* const dt_query_status_message(dt_query_status_t s) {
	...
	if(s >= DT_COUNT_OF(_DTQ_STATUS_MSG))
		return NULL;
...
dt_query_status_t dt_query(dt_value_t t, dt_command_t c, dt_value_t* ret) {
	...
	size_t i = 0;
	DT_ASSERT(t && c);
	cmd->target = (struct dt_value_t*)t;
