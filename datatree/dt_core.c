...
typedef struct _dt_datatree_detail_t {
	_dt_value_t root;
	xpl_context_t xpl;
	dt_pop_string_func_t string_popper;
	void* userdata;
...
xpl_status_t _dt_pop_xpl_string(xpl_context_t* _s, char** _o) {
	...
	_dt_datatree_detail_t* _dt = (_dt_datatree_detail_t*)(_s->userdata);
	if(_dt && (dt_datatree_t)_dt != DT_DUMMY_DATATREE) {
		if(_dt->string_popper) {
			return _dt->string_popper(_s, _o);
		}
	}
...
const char* const dt_status_message(dt_status_t s) {
	...
	if(s >= DT_COUNT_OF(_DT_STATUS_MSG))
		return NULL;
...
void dt_set_string_popper(dt_datatree_t d, dt_pop_string_func_t p) {
	_dt_datatree_detail_t* _dt = (_dt_datatree_detail_t*)d;
	if(_dt && (dt_datatree_t)_dt != DT_DUMMY_DATATREE) {
		_dt->string_popper = p;
	}
}
